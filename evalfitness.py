...
def FinalFitness(intermediate_outputs):
        ...
        ideal_results = np.array(settings.ideal_results)
        obtained_results = np.array(intermediate_outputs)
        
        if np.any(np.isinf(obtained_results)):
            return float('inf')
        
        # sum the absolute values of the differences over one example
        diff= np.sum(np.abs(ideal_results-obtained_results))
...
def FinalFitness2(intermediate_outputs):
        ...
        obtained_results=[intermediate_outputs[0], intermediate_outputs[0]]
