...
class KillProbError(Exception): pass
class OperatorProbError(Exception): pass
# Exceptions related to population size
class PopSizeError(Exception): pass


def EvolutionRun(popsize,root_node,mindepth,maxdepth,buildmethod,max_nb_runs, fitness_criterion ,crossover_prob,mutation_prob, kill_prob,size,prob_selection,dbname,verbose):
    """
...
#     ## fitness data output
#     f = open('data1.txt','a')
#     value = str(current_best_fitness)
#     f.write(value + ',')
#     f.close()
#==============================================================================
    ...
    if current_best_fitness<=fitness_criterion:
        ...
        while i<max_nb_runs and current_best_fitness>fitness_criterion:
            ...
            kill_size = np.floor(popsize*kill_prob)
            writepop.WriteInitialPopulation2DB(kill_size,root_node,mindepth,maxdepth,buildmethod,dbname,tablenames[i])
            TournamentSelectionEvolveDBPopulation2(popsize,maxdepth,crossover_prob,mutation_prob,kill_prob,size, prob_selection,dbname,tablenames[i-1],tablenames[i])
...
#==============================================================================
                ...
                con = sqlite.connect(dbname)
                SELECT = "select tree from %s where o_id=%d" % (tablenames[i-1],chosen_one[0])
                cur = con.cursor()
                cur.execute(SELECT)
                con.commit()
                myresult= cur.fetchone()
...
def TournamentSelectionEvolveDBPopulation2(popsize,maxdepth,crossover_prob,mutation_prob,kill_prob,size, prob_selection,dbname,tablename,tablename2):
    """
    ...
    if mutation_prob>1 or mutation_prob<0:
        ...
        exit
    if kill_prob>1 or kill_prob<0:
        raise KillProbError, "Crossover Probability should be in interval [0,1]"
        exit
    reproduction_prob= 1-(crossover_prob+mutation_prob+kill_prob)
    ...
    crossover_size= np.floor(popsize*crossover_prob)
    mutation_size= np.ceil(popsize*mutation_prob)
    reproduction_size= np.ceil(popsize*reproduction_prob)
    ...
    #con.execute("create table %s(o_id INTEGER PRIMARY KEY,tree TEXT,\
     #tree_mapping TEXT, treedepth INTEGER, evaluated INTEGER, fitness FLOAT)"%tablename2)
    #print len(new_pop)
    for i in xrange(0,popsize-int(np.floor(popsize*kill_prob))):
